                        ...
                        switch (result.resultCode) {
                            case "1":
                                ...
                                if(confirm("위시리스트에 등록되었습니다.\n위시리스트로 이동하시겠습니까?")) {
                                    top.location.href = "user/movielog/index.html";
                                }
                                break;
                            case "-1":
                                app.goLogin();
...
//                    config.end.val(cEnd);
    ...
    */
    ...
    $.fn.naverMap = function (options){
        ...
        // init Point.
        theaterPoint = new nhn.api.map.LatLng(theater.lat, theater.lng);
        // init Map.
        oMap = new nhn.api.map.Map('map', {
            ...
            minMaxLevel: [1, 14], // zoomIn, zoomOut
            size: new nhn.api.map.Size(config.w, config.h)
        });

        // 지도 버튼 : 일반/위성
        oMapTypeBtn = new nhn.api.map.MapTypeBtn();
        oMapTypeBtn.setPosition({ top: 5, right: 5 });
        oMap.addControl(oMapTypeBtn);

        // init Marker Iconf.
        iconSize = new nhn.api.map.Size(28, 37);
        iconOffset = new nhn.api.map.Size(14, 37);
        oIcon = new nhn.api.map.Icon('../img.cgv.co.kr/theater/pin_spot2.png', iconSize, iconOffset);

        // init Select Theater Maker.
//            oMarker = new nhn.api.map.Marker(oIcon, { title: theater.label });
//            oMarker.setPoint(theaterPoint);
//            oMarker.setVisible(true);
//            oMap.addOverlay(oMarker);
//  init Select Theater Label.
...
//            oLabel.setVisible(true, oMarker);
        ...
        config.data.forEach(function(v, i, a){
            ...
            oMarker.setPoint(new nhn.api.map.LatLng(v.lat, v.lng));
            oMarker.setVisible(true);
            oMap.addOverlay(oMarker);
...
//            if (oTarget instanceof nhn.api.map.MarkerLabel) {
//                app.log('eee');
//            }
//        });

        function setPoint(code) {
            var newTheater = config.data.filter(function (v, i, a) {
                    return v.code == code;
                })[0], 
                newPoint;
            
            newPoint = new nhn.api.map.LatLng(newTheater.lat, newTheater.lng);

            if(!newTheater || !newTheater.code) $.error('not defined theater info.');
            oMap.setCenter(newPoint);
        }
        return {
            move : setPoint
        };
             ...
             $this.on("click", function () {
                ...
                if (regionCode) {
                    ...
                    if(regionCode == "MovieCollage") {
                        url = "arthouse/index.html";     //무비꼴라쥬일 경우 무비꼴라쥬 페이지로 이동
                    } 
