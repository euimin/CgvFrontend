        ...
        if (app.config('isLogin')) {
            if ($(this).data('theatercode').length > 0) {
                ...
                if (regioncode == "103") {
					...
					//CINE KIDS 북수원
					location.href = "theaters/special/show-timesa807.html?regioncode=CK&amp;theatercode=0049";  //CINE KIDS 북수원
                } else {
                ...
                //ToDO : 윈도우 팝업창 공통화 하기
                var win = window.open("user/popup/favoritetheaters.html", "url", "left=0,top=0,width=645,height=370,toolbar=no,scrollbars=no");
                win.focus();
            }
        }
        else {
...
var moviediary = new function () {
    ...
    that.getPath = function (url) {
        ...
        return url.substring(url.indexOf('index.html'));
    }
